...
esp_err_t lcd_display_brightness_set(int brightness_percent) {
  ...
  ESP_ERROR_CHECK(
      ledc_set_duty(LEDC_LOW_SPEED_MODE, LCD_BACKLIGHT_LEDC_CH, duty_cycle));
...
esp_err_t lcd_display_rotate(lv_display_t *lvgl_disp,
                             lv_display_rotation_t dir) {
  if (lvgl_disp) {
...
esp_err_t app_lcd_init(esp_lcd_panel_io_handle_t *lcd_io,
  ...
  ESP_RETURN_ON_ERROR(
      spi_bus_initialize(LCD_SPI_HOST, &buscfg, SPI_DMA_CH_AUTO), TAG,
      "SPI init failed");
...
lv_display_t *app_lvgl_init(esp_lcd_panel_io_handle_t lcd_io,
                            esp_lcd_panel_handle_t lcd_panel) {
